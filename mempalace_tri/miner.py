...
def _compress_content(content: str, compressor: Compressor) -> tuple:
    """Compress content using the given compressor.

    Returns (compressed_content, compression_type) tuple.
    If compression fails, returns original content with type "none".
    """
    try:
        result = compressor.compress(content)
        if result and result.compressed and result.dialect_name:
            return result.compressed, result.dialect_name
        return content, "none"
    except Exception:
        return content, "none"


def _detect_compressor_mode(config: dict) -> CompressorMode:
    """Determine the compression mode from the project config.

    Falls back to auto-detect if no compression_mode is specified.
    """
    mode_str = config.get("compression_mode", "auto").lower().strip()
    mode_map = {
        "auto": CompressorMode.AUTO,
        ...
        "wenjian": CompressorMode.WENJIAN_SIMPLIFIED,
    }
    return mode_map.get(mode_str, CompressorMode.AUTO)
...
def _build_drawer_metadata(
    ...
    Args:
        compression_type: The compression dialect applied to this drawer's
            content (e.g., "aaak", "wenjian_zh_cn", "wenjian_zh_tw", "none").
    """
    metadata = {
        "wing": wing,
        "room": room,
...
def process_file(
    ...
    dry_run: bool,
    closets_col=None,
    compression_mode: CompressorMode = CompressorMode.AUTO,
) -> tuple:
    """Read, chunk, route, and file one file. Returns (drawer_count, room_name).

    Args:
        compression_mode: The compression mode to apply before chunking.
            When not "none", content is compressed with the matching dialect
            before being stored in drawers.
    """
    ...
    # Apply compression before chunking (if mode is not "none")
    compressor = Compressor(default_mode=compression_mode)
    compressed_content, compression_type = _compress_content(content, compressor)

    # Use compressed content for chunking
    chunks = chunk_text(compressed_content, source_file)

    if dry_run:
        comp_label = f" (compressed: {compression_type})" if compression_type != "none" else ""
        print(f"    [DRY RUN] {filepath.name} -> room:{room} ({len(chunks)} drawers){comp_label}")
    ...
    with mine_lock(source_file):
        ...
        if closets_col and drawers_added > 0:
            ...
            closet_lines = build_closet_lines(source_file, drawer_ids, compressed_content, wing, room)
            closet_id_base = (
                f"closet_{wing}_{room}_{hashlib.sha256(source_file.encode()).hexdigest()[:24]}"
            )
            entities = _extract_entities_for_metadata(content)  # entities from original
            closet_meta = {
                "wing": wing,
                "room": room,
                "source_file": source_file,
...
def mine(
    ...
    Args:
        compression_mode: Compression mode for mined drawers. One of
            "auto", "aaak", "wenjian", "wenjian_simplified",
            "wenjian_traditional". Defaults to "auto".
    """
...
def _mine_impl(
    ...
    # Determine effective compression mode: config > mine() arg > default
    effective_mode = compression_mode
    if effective_mode == "auto" and config.get("compression_mode"):
        effective_mode = config["compression_mode"]
    ...
    comp_label = f" [{effective_mode}]" if effective_mode != "auto" else ""
    if dry_run:
        ...
        for i, filepath in enumerate(files, 1):
            try:
                drawers, room = process_file(
                    ...
                    compression_mode=_detect_compressor_mode({"compression_mode": effective_mode}),
                )
