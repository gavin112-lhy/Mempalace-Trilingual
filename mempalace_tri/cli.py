...
def cmd_detect_lang(args):
    ...
    text = args.text
    detected = lang_detect.detect_language(text)
    ...
    label = lang_labels.get(detected, detected)
    print(f"  Detected language: {label} ({detected})")


def cmd_convert_s2t(args):
    """Convert between Simplified and Traditional Chinese."""
    from .compressor import lang_detect

    direction = args.direction or "s2t"
    text = args.text
    try:
        ...
        print(f"  {dir_labels.get(direction, direction)}:")
        print(f"    {result}")
    except ImportError:
        print("  ERROR: S2T converter not available. Install opencc or use Python 3.11+.", file=sys.stderr)
        sys.exit(1)
...
def main():
    ...
    p_mine.add_argument(
        "--compression-mode",
        default=None,
        choices=["auto", "aaak", "wenjian", "wenjian_simplified", "wenjian_traditional"],
        help="Compression mode for mined drawers (default: from config or 'auto')",
    )
    ...
    # detect-lang
    p_detect = sub.add_parser("detect-lang", help="Detect language of input text")
    p_detect.add_argument("text", help="Text to detect language for")

    # convert-s2t
    p_convert = sub.add_parser("convert-s2t", help="Convert between Simplified and Traditional Chinese")
    p_convert.add_argument("text", help="Text to convert")
    p_convert.add_argument(
        "--direction",
        choices=["s2t", "t2s"],
        default="s2t",
        help="Conversion direction: s2t (default) or t2s",
    )
