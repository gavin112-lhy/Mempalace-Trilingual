...
Usage:
    from mempalace_tri.entity_detector import detect_entities, confirm_entities
...
from mempalace_tri.i18n import get_entity_patterns
...
class EntityDetector:
    """Entity detector for project files.

    Provides a class-based API that wraps the module-level functions.
    Backward-compat alias for callers that instantiate ``EntityDetector()``.
    """
    ...
    def detect_entities(
        ...
        """Detect entities from file paths."""
        return detect_entities(file_paths, max_files, languages, corpus_origin)

    def scan_for_detection(self, project_dir: str, max_files: int = 10) -> list:
        """Scan directory for detectable files."""
        return scan_for_detection(project_dir, max_files)
