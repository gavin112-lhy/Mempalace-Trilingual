...
def tool_add_drawer(
    ...
    Optional compression_mode applies compression before storing:
    - null/none: no compression (default)
    - auto/aaak/wenjian/wenjian_simplified/wenjian_traditional: compress before storing
    """
    ...
    compressed = False
    compression_info = {}
    if compression_mode and compression_mode.lower() not in ("", "none", "null"):
        try:
            from .compressor import Compressor, CompressorMode
            mode_map = {
                "auto": None,
                "aaak": CompressorMode.AAAK,
                "wenjian": None,
                "wenjian_simplified": CompressorMode.WENJIAN_SIMPLIFIED,
                "wenjian_traditional": CompressorMode.WENJIAN_TRADITIONAL,
            }
            explicit_mode = mode_map.get(compression_mode.lower())
            compressor = Compressor(default_mode=explicit_mode)
    ...
    # Sanitize after compression (compression preserves valid UTF-8)
    try:
        wing = sanitize_name(wing, "wing")
        room = sanitize_name(room, "room")
    except ValueError as e:
    ...
    # Use compressed content for dedup ID to avoid false positives
    drawer_id = (
        f"drawer_{wing}_{room}_{hashlib.sha256((wing + room + content).encode()).hexdigest()[:24]}"
    )
    if compressed:
        drawer_id = f"comp_{compression_mode}_{drawer_id}"
    ...
    try:
        ...
        if compressed:
            meta["compression_mode"] = compression_mode
            meta["compression_ratio"] = compression_info.get("ratio", 0)
...
def tool_detect_lang(text: str):
    """Detect the language of the given text.

    Returns the detected language code (en, zh_cn, zh_tw, or mixed) and
    a human-readable label. Useful for understanding the corpus before
    compression or search.
    """
    from .compressor import lang_detect

    code = lang_detect.detect_language(text)
    labels = {
...
def tool_convert_s2t(text: str, direction: str = "s2t"):
    ...
    if direction not in ("s2t", "t2s"):
        return {"error": f"Invalid direction: {direction}. Use 's2t' or 't2s'."}

    try:
        from .compressor.wenjian import _s2t, _t2s
        converter = _s2t if direction == "s2t" else _t2s
        result = converter(text)
        dir_labels = {"s2t": "Simplified → Traditional", "t2s": "Traditional → Simplified"}
        return {
...
def tool_compression_test(text: str, mode: str = "auto"):
    """Test compression on the given text with the specified mode.

    Modes:
    - auto: Detect language and apply best compression
    - aaak: AAAK English compression
    - wenjian: Wenjian Chinese compression (auto-detects simplified/traditional)
    - wenjian_simplified: Wenjian with explicit Simplified Chinese
    - wenjian_traditional: Wenjian with explicit Traditional Chinese
    """
    from .compressor import Compressor, CompressorMode
    ...
    explicit_mode = mode_map.get(mode)
    ...
    try:
        ...
        return {
            ...
            "compressed_length": len(result.compressed),
            "compression_ratio": result.size_ratio,
            "detected_language": result.detected_language,
        }
    except Exception as e:
...
TOOLS = {
    ...
    "mempalace_add_drawer": {
        "description": "File verbatim content into the palace. Checks for duplicates first. Optional compression_mode applies compression before storing.",
        "input_schema": {
            "type": "object",
            "properties": {
                ...
                "compression_mode": {
                    "type": "string",
                    "description": "Compression mode: null/none for no compression, auto/aaak/wenjian/wenjian_simplified/wenjian_traditional for compression",
                },
            },
    ...
    # === Trilingual tools (Phase 5) ===
    "mempalace_detect_lang": {
        "description": "Detect the language of text (en, zh_cn, zh_tw, mixed). Useful before compression or when analyzing corpus.",
        "input_schema": {
            "type": "object",
            "properties": {
        ...
        "handler": tool_detect_lang,
    },
    "mempalace_convert_s2t": {
        "description": "Convert Chinese text between Simplified and Traditional Chinese. s2t: Simplified → Traditional, t2s: Traditional → Simplified.",
        "input_schema": {
            "type": "object",
            "properties": {
                ...
                "direction": {
                    "type": "string",
                    "description": "s2t (Simplified → Traditional) or t2s (Traditional → Simplified)",
                },
            },
            "required": ["text"],
        },
        "handler": tool_convert_s2t,
    },
    "mempalace_compression_test": {
        "description": "Test compression on text with a specific mode. Returns ratio, compressed output, and detected language. Modes: auto, aaak, wenjian, wenjian_simplified, wenjian_traditional.",
        "input_schema": {
            "type": "object",
            "properties": {
                ...
                "mode": {
                    "type": "string",
                    "description": "Compression mode: auto, aaak, wenjian, wenjian_simplified, wenjian_traditional",
                },
            },
            "required": ["text"],
        },
        "handler": tool_compression_test,
    },
}
