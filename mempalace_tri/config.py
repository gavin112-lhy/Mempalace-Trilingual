...
DEFAULT_COMPRESSION_MODE = "auto"  # auto | aaak | wenjian
DEFAULT_WENJIAN_SCRIPT = "simplified"  # simplified | traditional
...
class MempalaceConfig:
    ...
    # ── Compression settings (trilingual) ────────────────────────────────

    @property
    def compression_mode(self):
        """Compression dialect mode for mining.

        Values: ``"auto"`` (default, detect language and apply best dialect),
        ``"aaak"`` (English-only), ``"wenjian"`` (Chinese, uses wenjian_script).

        Reads ``MEMPALACE_COMPRESSION_MODE`` env first, then config file,
        then ``"auto"``.
        """
        env_val = os.environ.get("MEMPALACE_COMPRESSION_MODE")
        if env_val:
            return env_val.strip().lower()
        return str(self._file_config.get("compression_mode", DEFAULT_COMPRESSION_MODE)).strip().lower()

    def set_compression_mode(self, mode: str):
        """Persist compression mode to config.json."""
        mode = mode.strip().lower()
        valid_modes = ["auto", "aaak", "wenjian"]
        if mode not in valid_modes:
            raise ValueError(f"compression_mode must be one of {valid_modes}, got '{mode}'")
        self._file_config["compression_mode"] = mode
        self._config_dir.mkdir(parents=True, exist_ok=True)
        try:
            with open(self._config_file, "w", encoding="utf-8") as f:
                json.dump(self._file_config, f, indent=2, ensure_ascii=False)
    ...
    @property
    def wenjian_script(self):
        """Script variant for Wenjian compression when compression_mode is wenjian.

        Values: ``"simplified"`` (default), ``"traditional"``.
        """
        return str(self._file_config.get("wenjian_script", DEFAULT_WENJIAN_SCRIPT)).strip().lower()

    def set_wenjian_script(self, script: str):
        """Persist wenjian script variant to config.json."""
        script = script.strip().lower()
        valid_scripts = ["simplified", "traditional"]
        if script not in valid_scripts:
            raise ValueError(f"wenjian_script must be one of {valid_scripts}, got '{script}'")
        self._file_config["wenjian_script"] = script
        self._config_dir.mkdir(parents=True, exist_ok=True)
        try:
            with open(self._config_file, "w", encoding="utf-8") as f:
                json.dump(self._file_config, f, indent=2, ensure_ascii=False)
    ...
    def init(self):
        ...
        if not self._config_file.exists():
            default_config = {
                ...
                "wenjian_script": DEFAULT_WENJIAN_SCRIPT,
            }
