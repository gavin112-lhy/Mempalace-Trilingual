...
All notable changes to [MemPalace Trilingual](https://github.com/MemPalace/mempalace-trilingual) are documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.1.0/), and this project adheres to [Semantic Versioning](https://semver.org/).

---

## [3.3.4-tri.1] — Unreleased

> **MemPalace 3.3.4 → Trilingual merge: English (AAAK) + Chinese Simplified (文簡) + Chinese Traditional (文簡).**

### Added

- **Compression dialects — 3 languages, one palace**
  - AAAK English dialect (from mempalace-3.3.4) — entity codes, emotion markers, compressed structure
  - 文簡 Chinese Simplified dialect (文簡-简) — character-level compression for Simplified Chinese
  - 文簡 Chinese Traditional dialect (文簡-繁) — character-level compression for Traditional Chinese
  - Auto language detection — `detect_language()` returns `en`, `zh_cn`, `zh_tw`, or `mixed`
  - Pluggable dialect registry — `DialectRegistry` with `create_dialect()` factory

- **Simplified ↔ Traditional Chinese converter**
  - Full bidirectional S2T conversion with 15,000+ character mappings
  - `convert_text()` for single strings, `full_convert()` for file processing
  - Auto-detects source script on compression

- **Compression-aware mining**
  - `mempalace mine --compression-mode auto|aaak|wenjian|wenjian_simplified|wenjian_traditional`
  - Detected language determines which dialect is applied
  - Compression metadata stored per-drawer (mode, ratio, detected_language)

- **CLI language tools**
  - `mempalace detect-lang "text"` — detect language of input text
  - `mempalace convert-s2t "text" --direction s2t|t2s` — convert Chinese script

- **MCP trilingual tools**
  - `mempalace_detect_lang` — detect language (en/zh_cn/zh_tw/mixed)
  - `mempalace_convert_s2t` — bidirectional S2T conversion
  - `mempalace_compression_test` — test compression with any mode
  - `mempalace_add_drawer` — new `compression_mode` parameter

### Internal Changes

- **Config updates**
  - `MempalaceConfig.compression_mode` — palace-level compression setting
  - `MempalaceConfig.wenjian_script` — zh_cn or zh_tw for Wenjian
  - `MempalaceConfig.entity_languages` — multi-language entity detection

- **Miner pipeline**
  - `compress_text()` respects compression_mode from config
  - `convo_miner._extract_metadata` passes compression mode
  - Dedup extraction module from mempalace-3.3.4

- **MCP server**
  - Compression-aware `tool_add_drawer` — compresses before storing
  - `tool_compression_test` — test compression ratios across modes
  - `tool_detect_lang` — detect language before compression decisions
  - `tool_convert_s2t` — bidirectional Chinese script conversion
